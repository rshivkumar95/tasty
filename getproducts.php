<?php
	...
	if($action=="showAll"){
		...
		$query="SELECT * FROM `restaurant_menu` WHERE `Restaurant_Id`='$restaurant'";
		$result=mysql_query($query);
	}
	else if($action=="Name"){
		
		$query="SELECT * FROM `restaurant_menu` WHERE `Restaurant_Id`='$restaurant' ORDER BY `Item_Name`";
		$result=mysql_query($query);
	}
	else if($action=="Discount"){
		
		$query="SELECT * FROM `restaurant_menu` WHERE `Restaurant_Id`='$restaurant' ORDER BY `Discount` DESC";
		$result=mysql_query($query);
	}
	else if($action=="Price"){
		
		$query="SELECT * FROM `restaurant_menu` WHERE `Restaurant_Id`='$restaurant' ORDER BY `Item_price`";
		$result=mysql_query($query);
	}
	else if($action=="Popularity"){
		
		$query="SELECT * FROM `restaurant_menu` WHERE `Restaurant_Id`='$restaurant' ORDER BY `sellcount` DESC";
		$result=mysql_query($query);
	}
			...
			<li class="ajax_block_product col-xs-6 col-sm-6 col-md-2 ">
													<div class="product-container" itemscope="" >
														<div class="left-block">
															...
															<div class="actions quickview-block" >
																   <ul class="add-to-links">
																		...
																		<li >
																			<div class="compare">
																		       <a class="add_to_compare" title="Add to cart" onclick="addtocart(<?php echo $obj['Item_Id']; ?>)">Add to Cart</a>
																	        </div>
																		</li>
																	</ul>
														...
														<div class="right-block">
															...
															<?php if($obj['status']=='Available') echo "<h6 style='color:green;'>Available</h6>"; else echo "<h6 style='color:red;'>Un Available</h6>"; ?>
			 ...
			 <!--li class="ajax_block_product col-xs-6 col-sm-6 col-md-2 ">
							<div class="product-container" itemscope="" >
								<div class="left-block">
					<div class="product-image-container">
						<a class="product_img_link" onclick="viewproduct(<?php echo $obj['Item_Id']; ?>)" title="<?php echo $obj['Item_Name']; ?>" itemprop="url">
							<img class="replace-2x img-responsive" src="<?php echo $obj['Item_Image']; ?>" alt="<?php echo $obj['Item_Name']; ?>" title="<?php echo $obj['Item_Name']; ?>" itemprop="image">
						</a>
						<span class="new-box"><span class="new-label">New</span></span>
					</div>
					<div class="actions quickview-block">
						   <ul class="add-to-links">
								<li class="first">								
									<div class="wishlist">
										<a class="addToWishlist wishlistProd_1" title="Wishlist" onclick="WishlistCart(<?php echo $obj['Item_Id']; ?>);">
											Add to Wishlist
										</a>
									</div>
								</li>
								...
								<li class="last">
								   <button class="quick-view" onclick="viewproduct(<?php echo $obj['Item_Id']; ?>)" >
								   <span>Quick view</span>
							      </button>
								</li>
								<li >
									<div class="compare">
								       <button class="add_to_compare" title="Add to cart" onclick="addtocart(<?php echo $obj['Item_Id']; ?>)">Add to Cart</button>
							        </div>
								</li>
							</ul>
